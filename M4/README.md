...
2. 商人販售兩樣新商品：
   - **M4A1 步槍**（物品 ID 47930）：**1 元**。買到時已裝滿 30 發。
...
![六種瞄準鏡](Assets/m4-scopes.jpg)

| 瞄準鏡 | 物品 ID | 右鍵 | 開鏡方式 | 開鏡散布 |
|---|---|---|---|---|
| 不裝（機械瞄具） | 無 | 1.3× | 槍移到畫面中央，照門鬼環對準準星 | 60% |
| 紅點 | 47932 | 1.5× | 對準，紅點投在鏡筒中央 | 45% |
| 全像 | 47933 | 1.5× | 對準，紅圈加中心點 | 45% |
| ACOG（出廠） | 47934 | 4× → 收起 | 放大鏡畫面，紅色 chevron | 35% |
...
上圖是從眼睛位置離線渲染的三種 1× 瞄具，紅十字是畫面中心：照門鬼環、紅點鏡筒與全像窗口都對在前準星的尖端。
...
| `ScopeMagnification` | 4 | ACOG 倍率（其他鏡的倍率固定） |
...
- **瞄準鏡**：共用火槍瞄準鏡的 FOV、靈敏度與射擊時還原 FOV 的邏輯。ACOG 準星是紅色 chevron 加 400–800 m 落點刻度（遊戲沒有彈道下墜，刻度只是外觀）。火槍維持黃銅鏡與 3× → 6× → 收起。
- **可換鏡的狀態**：裝了哪種鏡存在槍的 `Item.metaInt` 低位元組。原生只有樂透彩券會寫這個欄位，它會跟著存檔（`SavedCont.items`、`SavedColl.item`）與 `Item.NetworkSerialize` 同步；9999 堆疊也會把值不同的槍分開放。值 0 是出廠 ACOG，所以 0.14.x 的舊槍不變。
...
- `tools/create-m4-model.js` 產生 `Assets/model.json`：144 個部件、4,480 三角面（同一時間只顯示一種鏡），全長 0.858 m（真槍 0.838 m），寬度加粗 18% 以配合遊戲的道具比例。
- 瞄準鏡各自一個 `role`（`scope-reddot`、`scope-holo`、`scope-acog`、`scope-brass`、`scope-sniper`），後照門分成立起（`iron-up`）和折下（`iron-down`，狙擊鏡座會蓋住所以不畫）。紅點是空心鏡筒、全像是開放窗口，對準時看得穿。
- `tools/render_m4.py` 離線渲染：`sheet` 概念圖、`icons` 背包圖示（槍、子彈、五種鏡，256×256）、`scopes` 六種配置對照、`ads` 1× 瞄具從眼睛看出去的畫面。
...
`run-m4.ps1` 包含：
- 123 項規則檢查：射速、換彈數量、RPC 合併與重入、散布、擊殺時間、瞄準段數、鏡的 `metaInt` 編碼、物品 ID 對應、安裝／拆卸／拆疊決策、各鏡參數、全像圈大小、合成音效。
- 9,870 項模型檢查：索引、非退化三角形、所有部件封閉且外向繞序、尺寸、圖示格式；照門鬼環中心與前準星尖端都在 `M4Scopes.cs` 的 `SightY = 0.091`，紅點在鏡筒軸線上、全像在窗口內，紅點沒有不透明鏡片。
- 133 項 Cecil 靜態檢查：遊戲 API、兩個原生限制仍存在、商店一整疊定價、拖曳會走到 `UseItemOnItem` 且原生只處理 1/2、`metaInt` 只有 Item 建構子與樂透會寫且有存檔與同步、`fpHands` 沒有新的原生使用者、FMOD／Netcode API、Harmony handler 參數、嵌入資源為最新版。
...
- 開鏡 4× 準星、開鏡射擊散布、換彈時自動收鏡；火槍 3×/6× 不受影響。
- 槍聲音量是否跟著遊戲音效滑桿；主機與客機互相聽到槍聲、看到槍口火光與第三人稱模型。
- 耐久用完損壞、修理；存檔後重讀彈匣數；死亡與斷線。
- 換鏡：把五種鏡各拖到槍上一次、在箱子裡換鏡、背包滿時換鏡；按 U 拆下；存檔重讀後鏡還在；疊在一起的兩把 M4 只換一把。
- 1× 對準：機械瞄具、紅點、全像開鏡時槍是否移到中央、準心對齊、後座力與換彈中斷；收鏡後切換其他工具位置是否正常。
- 放大鏡：ACOG 4×、黃銅 3×/6×、狙擊 3×/6×/9× 的倍率循環與準星。
...
- 其他玩家剛加入時，最多要等 3 秒才會看到持槍者裝的鏡（在那之前顯示 ACOG）。
