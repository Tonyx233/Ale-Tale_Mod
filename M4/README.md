...
3. 操作：
   ...
   - 打空時會自動換彈；按右鍵開 ACOG 4×，再按一次收起。
4. 右下角顯示「彈匣 / 容量　背包子彈　模式」。
...
| `ScopeMagnification` | 4 | ACOG 倍率 |
| `FireModeKey` | B | 切換射擊模式 |
| `SandFurniture` | false | 改用沙色 FDE 護木、槍托、彈匣 |
| `Volume` | 0.8 | 槍聲音量，會再乘上遊戲的音效音量 |
| `ModelScale` / `FirstPersonOffset` | 0.85 / `0,0,0` | 若手部位置對不上時微調第一人稱模型 |
...
- **模型**：第一人稱借用火槍的手、收放槍與換槍動畫，隱藏原版 mesh，把 M4 掛在 `MusketRoot/Musket`。第三人稱（`PlayerAnimTP`）與地上掉落物（`CollectibleNet`）也換成 M4，並套用火槍的持槍姿勢（`TorsoState = 3`）。
...
- **瞄準鏡**：共用火槍瞄準鏡的 FOV、靈敏度與射擊時還原 FOV 的邏輯。M4 使用單段 ACOG，準星是紅色 chevron 加 400–800 m 落點刻度（遊戲沒有彈道下墜，刻度只是外觀）；開鏡時散布縮小到 35%。火槍維持黃銅鏡與 3× → 6× → 收起。
- **散布**：首發 0.9，連射每發 +0.14，上限 3.2（原生 `spreadAngle` 單位），停火 0.12 秒後開始回復。

## 模型與素材

- `tools/create-m4-model.js` 產生 `Assets/model.json`：97 個部件、2,540 三角面，全長 0.858 m（真槍 0.838 m），寬度加粗 18% 以配合遊戲的道具比例。
- `tools/render_m4.py` 離線渲染概念圖與 256×256 背包圖示（`m4-icon.png`、`ammo-icon.png`）：`python M4/tools/render_m4.py sheet|icons`。
...
`run-m4.ps1` 包含：68 項射速／換彈數量／RPC 合併／散布／擊殺時間／瞄準段數／合成音效檢查；5,687 項模型檢查（索引、非退化三角形、所有部件封閉且外向繞序、尺寸對齊 `M4Model` 常數、圖示格式）；101 項 Cecil 靜態檢查，涵蓋遊戲 API、兩個原生限制仍存在、商店一整疊定價、FMOD / Netcode API、Harmony handler 參數與嵌入資源是否為最新版。
