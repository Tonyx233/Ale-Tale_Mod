...
def render(size, eye, target, scale, persp=None, colorway=None, hide=(), light=(.35, .8, .55), offset=(0, 0), gain=1.0):
    ...
    for pid, verts, tris, color, metal in world_parts(colorway, hide):
        cam = (verts - eye) @ R.T
        if persp:
            zc = -cam[:, 2]
