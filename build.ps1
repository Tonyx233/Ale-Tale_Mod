...
& $compiler /noconfig /nologo /target:library /optimize+ /nostdlib+ ('/out:'+(Join-Path $outDir 'Tony.TeammateHealthBars.dll')) @refs @resources (Join-Path $PSScriptRoot 'TeammateHealthBars\Plugin.cs') (Join-Path $PSScriptRoot 'YouTubeJukebox\YouTubeUrl.cs') (Join-Path $PSScriptRoot 'YouTubeJukebox\YouTubeJukeboxPanel.cs') (Join-Path $PSScriptRoot 'TavernCart\TavernCart.cs') (Join-Path $PSScriptRoot 'TavernCart\CartSeats.cs')
