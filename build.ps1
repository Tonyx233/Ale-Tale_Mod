...
& $compiler /noconfig /nologo /target:library /optimize+ /nostdlib+ ('/out:'+(Join-Path $outDir 'Tony.TeammateHealthBars.dll')) @refs @resources (Join-Path $PSScriptRoot 'TeammateHealthBars\Plugin.cs') (Join-Path $PSScriptRoot 'YouTubeJukebox\YouTubeUrl.cs') (Join-Path $PSScriptRoot 'YouTubeJukebox\YouTubeJukeboxPanel.cs') @(Get-ChildItem (Join-Path $PSScriptRoot 'Horse') -Filter '*.cs' | ForEach-Object FullName) ('/resource:'+(Join-Path $PSScriptRoot 'Horse\Assets\model.json')+',Tony.Horse.model.json')
