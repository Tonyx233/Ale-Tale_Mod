...
& $compiler /noconfig /nologo /target:library /optimize+ /nostdlib+ ('/out:'+(Join-Path $outDir 'Tony.TeammateHealthBars.dll')) @refs @resources (Join-Path $PSScriptRoot 'TeammateHealthBars\Plugin.cs') @(Get-ChildItem (Join-Path $PSScriptRoot 'GunScope') -Filter '*.cs' | ForEach-Object FullName) @(Get-ChildItem (Join-Path $PSScriptRoot 'ItemStacks') -Filter '*.cs' | ForEach-Object FullName) @(Get-ChildItem (Join-Path $PSScriptRoot 'QuickStack') -Filter '*.cs' | ForEach-Object FullName) (Join-Path $PSScriptRoot 'YouTubeJukebox\YouTubeUrl.cs') (Join-Path $PSScriptRoot 'YouTubeJukebox\YouTubeJukeboxPanel.cs') (Join-Path $PSScriptRoot 'YouTubeJukebox\JukeboxSpeaker.cs') (Join-Path $PSScriptRoot 'YouTubeJukebox\JukeboxState.cs') @(Get-ChildItem (Join-Path $PSScriptRoot 'Horse') -Filter '*.cs' | ForEach-Object FullName) ('/resource:'+(Join-Path $PSScriptRoot 'Horse\Assets\model.json')+',Tony.Horse.model.json') ('/resource:'+(Join-Path $PSScriptRoot 'Horse\Assets\model2.json')+',Tony.Horse.model2.json')
