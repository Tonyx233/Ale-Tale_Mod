...
namespace TonyMods
{
    [BepInPlugin("Tony.TeammateHealthBars", "Tony Ale & Tale Mods", "0.17.2")]
    ...
    public sealed class TeammateHealthBars : BaseUnityPlugin
    {
        ...
        private void Awake()
        {
            try { ItemStacks.Initialize(Logger); }
            catch (Exception ex) { Logger.LogError("9999 stacks disabled; stack patch initialization failed: " + ex); }
            ...
            Logger.LogInfo("Tony Ale & Tale Mods 0.17.2 loaded (health panel + YouTube jukebox + original horse + five-seat Horse 2 + X horse storage + musket scope + chest quick stack + M4A1 rifle with interchangeable optics + Tidefork summons with water shell volleys).");
            ...
            gameObject.AddComponent<ChestQuickStack>().Initialize(Config, Logger);
            TideSummons tide = gameObject.AddComponent<TideSummons>();
            try { tide.Initialize(Config, Logger); }
            catch (Exception ex) { Logger.LogError("Tidefork disabled: " + ex); Destroy(tide); }
            M4Armory armory = gameObject.AddComponent<M4Armory>();
            try { armory.Initialize(Config, Logger); }
            catch (Exception ex) { Logger.LogError("M4A1 rifle disabled; initialization failed: " + ex); Destroy(armory); }
        }
