...
namespace TonyMods
{
    ...
    {
        private readonly float[] stages;
        private int index = -1;
        public ScopeCycle() : this(3, 6) { }
        public ScopeCycle(params float[] stages)
        {
            if (stages == null || stages.Length == 0) throw new ArgumentException("Scope needs at least one stage");
            this.stages = (float[])stages.Clone();
        }
        public float Magnification { get { return index < 0 ? 1 : stages[index]; } }
