...
- 0.14.0 起與 M4A1 共用：M4 由 `Musket_fp` 生成，同樣有火槍 mesh，所以先以 `M4Armory.IsM4` 判斷；M4 不加火槍的黃銅鏡，倍率段數與準星依裝的鏡決定（`M4Scopes`：機械瞄具、紅點 1× 對準；黃銅鏡、狙擊鏡放大），開鏡時 M4 散布縮小。詳見 [M4 README](../M4/README.md)。
