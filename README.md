...
- 依新版三視圖重製頸冠、凹孔、倒角胸板、立體橫魚與根腳；加入青銅鏽紋與紅石裂紋貼圖，取消胸口青色發光縫。
- 8 個動畫關節、70 部件、2,388 triangles，兩張 512×512 貼圖嵌入 DLL。實際模型預覽：[三視角](Tidefork/Assets/model-preview.png)。
- 通過 build、幾何／UV、資產打包及十魚架規則／API 檢查；尚未部署或遊戲內實測。

## 0.17.1 十魚架潮彈改為五連射

- 潮彈**一次連射 5 發**：蓄力 0.7 s 後每 0.35 s 擲出一發，每發在前一發擲出時才瞄準當下最近的玩家；**5 發擲完才進冷卻**，整輪站著不動，擲完立刻繼續追。
- 數值：每發 **14 傷害**（原 16）＋緩速 −30% 移動速度 3 秒；冷卻 **2.5 s**（原 5 s）；射程 **7–29 m**（原 7–22 m）；水彈飛行速度 **+50%**（7 m ≈ 0.76 s、29 m ≈ 1.24 s）；落點半徑 **+30%** 為 2.86 m（預告圈 3.25 m）。同一輪最多可吃到 5 發共 70 傷害；緩速重新計時、不疊加。
- 同步協定升為 `Tony.Tidefork.v4`（快照每 3 隻一包），**所有玩家需同為 0.17.1**。已通過 build 與離線規則、API／協定、既有功能回歸；**連射尚未遊戲內實測**。[數值與實作](Tidefork/README.md)

## 0.17.0 十魚架潮彈（遠程技能）

- 十魚架新增遠程技能**潮彈**：目標在 **7–22 m**（站在牠走不到的地方時 0–22 m）且近戰打不到時，站定 0.7 s 在頭冠上聚出水球，向目標拋射；落點地面先畫出預告圈，內圈擴到外圈時落地。
- 落地 **2.2 m** 內的玩家受 **16 傷害**，並被**緩速 −30% 移動速度 3 秒**（原生效果，重複命中不疊加）；從天而降不能格擋，牆與天花板擋得住。出場 3 s 後可首發，之後 **5 s** 冷卻。**擲出後立刻繼續追**，水彈在空中自己飛。
- 預判目標移動的一半（最多 4 m），站著不動或慢慢走會被打中，跑動可以甩開。十魚架死亡時空中水彈消失。
- 同步協定升為 `Tony.Tidefork.v3`（快照每 5 隻一包，確保單包不超過傳輸層 6 KB 上限），**所有玩家需同為 0.17.0**。已通過 build 與離線規則、API／協定、既有功能回歸；**潮彈尚未遊戲內實測**。[數值與實作](Tidefork/README.md)

## 0.16.5 叉潮像改名為十魚架

- 怪物改名為**十魚架**（原叉潮像），商店物品改名為**十魚架球**；商店描述改為三行：「十魚架／天野 裕夫／平成元年3月」。名稱與描述所有語言相同，召喚提示文字同步改名。數值與行為不變。

## 0.16.4 牛馬換位改 Ctrl+數字

- 牛馬與牛馬2 騎乘時換位改為 **Ctrl+1～5**（原 Ctrl+F1～F5）：Ctrl+1 駕駛位，Ctrl+2～5 乘客位（原牛馬只有 1／2）。
- 騎乘中按 Ctrl+1～5 **不會切換快捷列、不會換掉手上的裝備**；沒按 Ctrl 的數字鍵、下馬後的 Ctrl+數字照原生行為換裝備。horse protocol 不變，可與 0.16.3 共存。

## 0.16.3 十魚架數值再調整

- 移動在 0.16.2 基礎上**再 +50%**：速度 **4.725 m/s**、加速度 15.75、轉身 360°/s、停下距離 3.7125 m、出招距離 4.8375 m、橫掃切換距離 3.2625 m；**每 0.15 秒重新尋路**（原 0.3）。
- 索敵範圍改回 30 m／高度差 4 m；三招前搖改回原本的 0.7／1.0／1.6 秒。
- 攻擊範圍：突咬前方 **6 m**（寬 2.6 m 不變）、橫掃扇形 **3.5 m**、震波 **6 m**；預告線與特效同步。HP 1000、無上限、無時間限制、傷害與冷卻不變。

## 0.16.2 十魚架數值調整

- 十魚架**移除全場 8 隻上限與 120 秒存活時間**：不會自己消失，只會被擊殺，或在讀檔、新遊戲、換場景、離房時清除。每位玩家召喚仍需間隔 1 秒。
- **HP 180 → 1000**；移動全部 +50%（速度 3.15 m/s、加速度 10.5、轉身 240°/s、索敵 45 m／高度差 6 m、出招與停下距離）；三招**前搖減半**（0.35／0.5／0.8 s，收招不變）；攻擊**範圍加倍**（突咬 4.6×2.6 m、橫掃 5.2 m、震波 8 m），預告線與水波特效同步放大。傷害與震波冷卻不變。
- 怪物數量不設上限後，同步快照改為每 12 隻一包（`Tony.Tidefork.v2`），所有玩家需同版本。平衡數值集中在 [`Tidefork/TideRules.cs`](Tidefork/TideRules.cs)。
...
- 模型拿掉兩種鏡的部件（144 → 125 部件），背包圖示改為機械瞄具。116 項規則、8,341 項模型、131 項 API／打包檢查與瞄準鏡、十魚架、一鍵放入、堆疊回歸通過。**尚未遊戲內實測**。

## 0.16.0 十魚架召喚球

- 商人新增「十魚架球」：**1 金／1 顆**。背包使用後向前拋出約 5 m（被擋時改試 4／3／6 m），落地召喚自製青銅怪物；長頸冠枝、紅色胸板、橫魚與根掌腳沿用照片設計。
- 十魚架會追打所有玩家（包含召喚者），具有魚吻突咬、銅尾橫掃與胸潮震波；180 HP，可被原生武器擊殺；120 秒消失、全場最多 8 隻，不掉戰利品。
...
- 雙人馬：商人購買，背包使用後在戶外放置；1 位駕駛＋1 位乘客，Ctrl+1/2 換位，附帶存檔；無人騎乘時對準馬長按 X 收回背包。[操作與限制](Horse/README.md)
